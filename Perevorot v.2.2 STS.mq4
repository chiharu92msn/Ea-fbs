...
extern double RiskPercent = 100.0;
...
void init() {
   ...
   fSplitStrToStr(TicketsList, gsa_140, "/");
   return (0);
}
   ...
   for (int l_pos_20 = 0; l_pos_20 < OrdersTotal(); l_pos_20++) {
      if (OrderSelect(l_pos_20, SELECT_BY_POS, MODE_TRADES)) {
         if (OrderSymbol() == Symbol()) {
            for (int l_index_24 = 0; l_index_24 < ArraySize(gsa_140); l_index_24++) {
               if (fStrUCase(gsa_140[l_index_24]) == "ALL" || StrToInteger(gsa_140[l_index_24]) == OrderTicket()) {
                  ThreeLevelSystemOfOutput();
                  l_digits_8 = MarketInfo(OrderSymbol(), MODE_DIGITS);
                  ld_unused_0 = MarketInfo(OrderSymbol(), MODE_POINT);
                  ls_12 = ls_12 + OrderSymbol() + "  Price: " + DoubleToStr(OrderOpenPrice(), l_digits_8) + "  SL = " + DoubleToStr(OrderStopLoss(), l_digits_8) + " (" + StopLossInPoint() + ")\n";
               }
            }
         }
      }
...
void CloseOneThird() {
   ...
   if (l_lots_4 > 0.0) {
      ...
      if (l_ord_close_0 && UseSound) PlaySound(gs_132);
   }
}
...
void TrailingPositions() {
   ...
   if (OrderType() == OP_BUY) {
      ...
      if (ND(l_bid_0 - OrderOpenPrice()) > ND(TrailingStop * l_point_16)) {
         if (ND(OrderStopLoss()) < ND(l_bid_0 - (TrailingStop + TrailingStep - 1) * l_point_16)) {
   ...
   if (OrderType() == OP_SELL) {
      ...
      if (ND(OrderOpenPrice() - l_ask_8) > ND(TrailingStop * l_point_16))
         if (ND(OrderStopLoss()) > ND(l_ask_8 + (TrailingStop + TrailingStep - 1) * l_point_16) || ND(OrderStopLoss()) == 0.0) ModifyStopLoss(l_ask_8 + TrailingStop * l_point_16);
   }
}
...
void ModifyStopLossInPoint(int ai_0) {
   double l_price_8 = 0;
   double l_point_16 = MarketInfo(OrderSymbol(), MODE_POINT);
   if (OrderType() == OP_BUY) l_price_8 = OrderOpenPrice() + ai_0 * l_point_16;
   if (OrderType() == OP_SELL) l_price_8 = OrderOpenPrice() - ai_0 * l_point_16;
   int l_bool_4 = OrderModify(OrderTicket(), OrderOpenPrice(), l_price_8, OrderTakeProfit(), 0, CLR_NONE);
...
int ProfitPosition() {
   ...
   if (OrderType() == OP_BUY) {
      l_bid_0 = MarketInfo(OrderSymbol(), MODE_BID);
      ld_24 = ND0((l_bid_0 - OrderOpenPrice()) / l_point_16);
   }
   if (OrderType() == OP_SELL) {
      l_ask_8 = MarketInfo(OrderSymbol(), MODE_ASK);
      ld_24 = ND0((OrderOpenPrice() - l_ask_8) / l_point_16);
   }
...
int StopLossInPoint() {
   ...
   double ld_8 = 0;
   if (OrderType() == OP_BUY) ld_8 = ND0((OrderStopLoss() - OrderOpenPrice()) / l_point_0);
   if (OrderType() == OP_SELL) ld_8 = ND0((OrderOpenPrice() - OrderStopLoss()) / l_point_0);
   if (OrderStopLoss() == 0.0) ld_8 = (-OrderOpenPrice()) / l_point_0;
   return (MathRound(ld_8));
}

double ND0(double ad_0) {
   return (NormalizeDouble(ad_0, 0));
}


string fStrUCase(string as_0) {
   int li_12;
   for (int li_8 = 0; li_8 < StringLen(as_0); li_8++) {
      li_12 = StringGetChar(as_0, li_8);
      if (li_12 > '`' && li_12 < '{') as_0 = StringSetChar(as_0, li_8, li_12 - 32);
      else {
         if (li_12 > 'Я' && li_12 < 256) as_0 = StringSetChar(as_0, li_8, li_12 - 32);
         else
            if (li_12 == 'ё') as_0 = StringSetChar(as_0, li_8, 'Ё');
      }
   }
   return (as_0);
}
